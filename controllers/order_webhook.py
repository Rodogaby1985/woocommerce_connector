...
class WooCommerceOrderWebhook(http.Controller):
    ...
    def receive_order_created(self):
        ...
        backends = backend_model._get_order_webhook_backends()
        if not backends:
            _logger.info('Webhook order.created ignorado: no hay backend conectado')
        ...
        if not verified_backend:
            ...
            message = 'Firma webhook inválida para order.created'
